...
(We also tried real peer-to-peer video between players — WebRTC, everyone sees everyone's actual camera — but pulled it back out. It's a big feature to get right without multiple real browsers to test against, and the one blocker we did hit (video stuck "connecting" across different networks) needs a TURN server, which needs its own account/setup. The commit is still in git history if it's worth revisiting later.)
